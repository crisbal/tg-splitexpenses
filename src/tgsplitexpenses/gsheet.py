...
class GSheet:
    ...
    def _create_row_from_transaction(transaction: Transaction, app_config: AppConfig) -> list:
        ...
        row = [
            ...
            *[transaction.split_type.split[user.id]/100 for user in app_config.expenses.users], # percentage
            *[share_by_user[user.id] for user in app_config.expenses.users], # money in transaction
            *[0 if user.id == transaction.paid_by.id else share_by_user[user.id] for user in app_config.expenses.users], # transaction debt
        ]
        return row
    ...
    def get_debtor(self, app_config: AppConfig) -> typing.Tuple[str, float]:
        user_in_debt = self.summary_worksheet.acell(app_config.gsheet.summary_worksheet_cell_user_in_debt).value
        amount_to_repay = self.summary_worksheet.acell(app_config.gsheet.summary_worksheet_cell_amount_to_repay).value
